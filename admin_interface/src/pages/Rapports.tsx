...
import { TrendingUp, CreditCard, PiggyBank, ArrowLeftRight, Download, FileText, ChevronDown, LucideIcon } from "lucide-react";
...
// Fonction universelle pour formater les valeurs dans les tooltips
const formatTooltipValue = (value: unknown): string => {
  ...
  if (Array.isArray(value) && value.length > 0) {
    ...
    if (typeof firstValue === 'number') {
      return new Intl.NumberFormat("fr-FR").format(firstValue) + " FCFA";
    }
...
const Rapports = () => {
  ...
  const donneesStatutsCredits: DonneesStatutsCredits[] = [
    { nom: t.credits?.actif || "Actif",           valeur: credits.filter((c: Credit) => c.statut === "en_cours").length },
    { nom: t.credits?.rembourse_statut || "Remboursé", valeur: credits.filter((c: Credit) => c.statut === "rembourse").length },
    { nom: t.credits?.enAttente_statut || "En attente", valeur: credits.filter((c: Credit) => c.statut === "en_attente").length },
    { nom: t.credits?.enRetard_statut || "En retard", valeur: credits.filter((c: Credit) => c.statut === "en_retard").length },
  ];
  ...
  const donneesCanaux: DonneesCanaux[] = [
    {
      ...
      montant: transactions.filter((tx: Transaction) => tx.canal === "Orange Money").reduce((acc, tx) => acc + tx.montant, 0),
    },
    {
      canal: "MTN Mobile Money",
      transactions: transactions.filter((tx: Transaction) => tx.canal === "MTN Mobile Money").length,
      montant: transactions.filter((tx: Transaction) => tx.canal === "MTN Mobile Money").reduce((acc, tx) => acc + tx.montant, 0),
    },
  ];

  const indicateurs: Indicateur[] = [
    { label: t.rapports?.tauxRemboursement || "Taux de remboursement",    valeur: 94,            couleur: "#16a34a" },
    { label: t.rapports?.transactionsReussies || "Transactions réussies", valeur: tauxReussite,  couleur: "#0891b2" },
    { label: t.rapports?.epargneReguliere || "Épargne régulière",         valeur: 72,            couleur: "#F59E0B" },
    { label: t.rapports?.creditsEnRetard || "Crédits en retard",          valeur: 17,            couleur: "#ef4444" },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="relative" ref={menuRef}>
          ...
          {menuExportOuvert && (
            <div
              className="absolute right-0 mt-2 w-52 bg-white dark:bg-gray-900 rounded-xl
              border border-gray-100 dark:border-gray-800 z-20 overflow-hidden"
          ...
          return (
            <div key={item.id} className="carte flex items-center gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xl font-bold text-gray-800 dark:text-white mt-0.5 truncate">{item.valeur}</p>
        ...
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={donneesGraphique} margin={{ top: 5, right: 10, bottom: 5, left: 0 }}>
            ...
            <Tooltip
              ...
              contentStyle={{ borderRadius: "12px", border: "1px solid #e5e7eb", fontSize: "13px", boxShadow: "0 4px 12px rgba(0,0,0,0.1)" }}
            />
            <Legend iconType="circle" iconSize={8} />
            <Line type="monotone" dataKey="credits"      stroke="#16a34a" strokeWidth={2.5} dot={{ r: 4, fill: "#16a34a" }} activeDot={{ r: 6 }} name={t.navigation?.credits || "Crédits"} />
            <Line type="monotone" dataKey="epargnes"     stroke="#F59E0B" strokeWidth={2.5} dot={{ r: 4, fill: "#F59E0B" }} activeDot={{ r: 6 }} name={t.navigation?.epargnes || "Épargnes"} />
            <Line type="monotone" dataKey="transactions" stroke="#0891b2" strokeWidth={2.5} dot={{ r: 4, fill: "#0891b2" }} activeDot={{ r: 6 }} name={t.navigation?.transactions || "Transactions"} />
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        {/* Répartition crédits */}
        <div className="carte">
          <h3 className="titre-section mb-1">{t.rapports?.repartitionCredits || "Répartition des Crédits"}</h3>
          <p className="sous-titre mb-4">{t.rapports?.parStatutRemboursement || "Par statut de remboursement"}</p>
        ...
        <div className="carte">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={donneesCanaux} barSize={36}>
              ...
              />
              <Legend iconType="circle" iconSize={8} />
              <Bar dataKey="transactions" fill="#f97316" name={t.rapports?.nbTransactions || "Nb. Transactions"} radius={[6, 6, 0, 0]} />
              <Bar dataKey="montant"      fill="#16a34a" name={t.rapports?.montantFCFA || "Montant (FCFA)"}    radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
