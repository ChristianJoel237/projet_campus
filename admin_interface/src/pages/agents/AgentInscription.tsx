...
const AgentInscription = () => {
    ...
    const getPlaceholder = (field: string): string => {
        const placeholders: Record<string, string> = {
            ...
            telephone: t.agentsTerrain?.telephonePlaceholder || "+237 6XX XXX XXX",
            ville: t.agentsTerrain?.zonePlaceholder || "Sélectionner ou saisir la ville",
            email: t.agentsTerrain?.emailPlaceholder || "nom@domaine.com",
            motDePasse: t.agentsTerrain?.motDePassePlaceholder || "Minimum 8 caractères",
            confirmer: t.agentsTerrain?.confirmerPlaceholder || "Ressaisir le mot de passe",
        };
        return placeholders[field] || "";
    ...
    const handleSoumettre = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setTimeout(() => {
                navigate("/agents", {
                    state: { message: "✅ Agent créé avec succès !" },
                    replace: true,
                });
            }, 1500);
        } catch (err: any) {
            setStatut("erreur");
            setMessageErreurAPI(err?.message || "Une erreur est survenue lors de la création de l'agent.");
        }
    };
                        ...
                        <div className="space-y-4">
                                    ...
                                    <div className="flex gap-1">
                                        {[1, 2, 3, 4].map((level) => (
                                            <div
                                                ...
                                                style={{
                                                    background:
                                                        formData.motDePasse.length >= level * 2
                                                            ? level <= 2
                                                                ? "#f97316"
                                                                : "#16a34a"
                                                            : "#e5e7eb",
                            ...
                            <div className="flex flex-col sm:flex-row gap-3 pt-4">
                                ...
                                >
                                    {estEnChargement
                                        ? "Création en cours..."
                                        : t.agentsTerrain?.creer || "Créer le compte"}
                                </button>
                            </div>
                        </div>
                    </form>
