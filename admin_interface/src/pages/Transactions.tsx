...
const Transactions = () => {
    const { t } = useLangue();
    const { data: transactionsServeur } = useTransactions();
    const [recherche, setRecherche] = useState<string>("");
    const [filtreType, setFiltreType] = useState<string>("tous");
    const [filtreCanal, setFiltreCanal] = useState<string>("tous");
    const [listeTransactions, setListeTransactions] = useState<Transaction[]>([]);

    useEffect(() => {
        if (transactionsServeur) {
            setListeTransactions(transactionsServeur);
        }
    }, [transactionsServeur]);

    const transactionsFiltrees = listeTransactions.filter((tx) => {
    ...
    const totalDepots = listeTransactions
        .filter((t) => t.type === "depot" && t.statut === "reussi")
        .reduce((acc, t) => acc + t.montant, 0);
    const totalTransferts = listeTransactions
        .filter((t) => t.type === "transfert" && t.statut === "reussi")
        .reduce((acc, t) => acc + t.montant, 0);
    const totalRetraits = listeTransactions
        .filter((t) => t.type === "retrait" && t.statut === "reussi")
        .reduce((acc, t) => acc + t.montant, 0);
    const totalRemboursements = listeTransactions
        .filter((t) => t.type === "remboursement" && t.statut === "reussi")
        .reduce((acc, t) => acc + t.montant, 0);

    const cartesResume: CarteResume[] = [
        {
            id: "depots",
            label: t.transactions?.totalDepots || "Total Dépôts",
            valeur: `${new Intl.NumberFormat("fr-FR").format(totalDepots)} FCFA`,
            icone: ArrowDownLeft,
            couleur: "#16a34a",
        ...
        {
            id: "transferts",
            label: t.transactions?.totalTransferts || "Total Transferts",
            valeur: `${new Intl.NumberFormat("fr-FR").format(totalTransferts)} FCFA`,
            icone: RefreshCw,
            couleur: "#0891b2",
        ...
        {
            id: "retraits",
            label: t.transactions?.totalRetraits || "Total Retraits",
            valeur: `${new Intl.NumberFormat("fr-FR").format(totalRetraits)} FCFA`,
            icone: ArrowUpRight,
            couleur: "#f97316",
            fond: "rgba(249,115,22,0.1)",
        },
        {
            id: "remboursements",
            label: t.transactions?.remboursements || "Remboursements",
            valeur: `${new Intl.NumberFormat("fr-FR").format(totalRemboursements)} FCFA`,
            icone: TrendingUp,
            couleur: "#8b5cf6",
    ...
    const iconeType = (type: TypeTransaction): JSX.Element | null => {
        switch (type) {
            case "depot":
                return <ArrowDownLeft size={14} style={{ color: "#16a34a" }} aria-hidden="true" />;
            case "retrait":
                return <ArrowUpRight size={14} style={{ color: "#f97316" }} aria-hidden="true" />;
            case "transfert":
                return <RefreshCw size={14} style={{ color: "#0891b2" }} aria-hidden="true" />;
            case "remboursement":
                return <TrendingUp size={14} style={{ color: "#8b5cf6" }} aria-hidden="true" />;
            default:
                return null;
        }
    };
            ...
            <div className="table-container">
                ...
                <div className="pied-tableau">
                    <p className="text-xs font-medium text-gray-600 dark:text-gray-300">
                        {transactionsFiltrees.length} {t.commun?.trouve || "trouvé(s)"} {t.commun?.sur || "sur"}{" "}
                        {listeTransactions.length} {t.commun?.total || "total"}
                    </p>
                </div>
            </div>
        </div>
