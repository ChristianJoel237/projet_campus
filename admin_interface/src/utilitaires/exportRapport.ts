...
<<<<<<< HEAD
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      [
        ...
        statistiquesGenerales.transactionsAujourdhui,
      ],
  ...
  autoTable(doc, {
    startY: positionY,
    head: [
      ['Commercant', 'Montant', 'Rembourse', 'Taux', 'Echeance', 'Statut'],
    ],
      ...
      const cells = ligne
        .map((val, c) => {
          ...
          if (colonnesMontant.includes(c)) {
            const montant =
              typeof val === 'number' ? formaterMontant(val) : val;
...
<<<<<<< HEAD
  ...
  const statsHTML = creerTableHTML({
    ...
    lignes: [
      ...
      [
        'Revenus Total (FCFA)',
        formaterMontant(statistiquesGenerales.revenusTotal),
      ],
      ['Taux de Remboursement', `${statistiquesGenerales.tauxRemboursement}%`],
    ],
    colonnesMontant: [],
